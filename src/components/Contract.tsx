...
const Contract = (props:PropsWithChildren<IContract>) => {
    const {children, project, timePeriod, contractValue, vendor, progress, contractValueMore, timePeriodMore, vendorMore} = props;
    ...
    return (
        <div style={moreActive ? {minHeight: "230px"} : {minHeight:"46px"}}className="contract-container trans-03s">
            <div className="flex col relative">
                ...
                <div style={moreActive ? {opacity: "100%"} : {}} className="absolute mt-80px more-contract-info flex col trans-03s opacity-0">
                    {vendorMore.map((s, idx) => 
                    moreActive && <span key={idx} className="my-2px color-light-gray font-14px trans-03s">
                        {s}
                    </span>)}
                </div>
            </div>
